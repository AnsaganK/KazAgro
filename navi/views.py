...
def archiveAgroHym(request):
    ...
    for i in list(request.user.groups.all()):
        if str(i) == title:
            hasGroup = True
    info = Agrohym.objects.all().order_by('-pk').filter(date__lt=fdate,status='Готово')
    paginator = Paginator(info, 20)  # Show 25 contacts per page
    page = request.GET.get('page')
    try:
        data = paginator.page(page)
...
def addSample(request):
    if request.method == "POST":
        ...
        if (zero+count)>client.countSamples:
            ...
            elements = elementsName.objects.all()
            errorMax = True
            return render(request, 'samples.html', {'samples':samples, 'clients':clients, 'elements':elements, 'errorMax':errorMax})
...
def newTable(request):
    ...
    for i1 in clients:
        ...
        dic1[(icount,i1.name+" | "+str(i1.countSamples),full(dic2,i1.countSamples))] = dic2
    #print(dic1)
